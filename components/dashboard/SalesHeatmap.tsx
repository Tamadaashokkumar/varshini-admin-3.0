...
export default function SalesHeatmap() {
  const [data, setData] = useState<any[]>([]);
  const [mounted, setMounted] = useState(false);
  // 🔥 Performance Optimization: Delay map rendering
  ...
  useEffect(() => {
    ...
    // 🔥 Performance Hack: Wait for 2.5 seconds before loading the heavy map
    // This allows the main thread to be free for Lighthouse LCP/TBT calculation
  ...
  // If not mounted OR map shouldn't load yet, show Skeleton
  if (!mounted || !shouldLoadMap)
    return (
