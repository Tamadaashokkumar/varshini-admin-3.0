...
export default function Header({
  ...
  const profile = user || {}; // Use passed user prop
  ...
  const handleLogout = async () => {
    try {
      await logout(); // Call global logout
        ...
        <div className="flex h-16 items-center justify-between px-4 lg:px-6">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="rounded-md p-2 text-zinc-500 hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-white/10 lg:hidden"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center gap-3 rounded-full border border-zinc-200 bg-white p-1 pr-3 hover:bg-zinc-50 dark:border-white/10 dark:bg-white/5 dark:hover:bg-white/10 transition-all"
              >
              ...
              <AnimatePresence>
                {isProfileOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 w-64 origin-top-right overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-xl ring-1 ring-black/5 dark:border-white/10 dark:bg-zinc-900 dark:shadow-2xl"
                  >
                    <div className="px-4 py-3 border-b border-zinc-100 dark:border-white/5 bg-zinc-50/50 dark:bg-white/5">
                      ...
                      {profile?.role === "superadmin" && (
                        <div className="mt-2 inline-flex items-center gap-1.5 rounded-md bg-blue-50 px-2 py-1 text-[10px] font-bold text-blue-700 ring-1 ring-inset ring-blue-700/10 dark:bg-blue-400/10 dark:text-blue-400">
            ...
            >
              <div className="flex flex-col items-center text-center">
                ...
                <div className="grid w-full grid-cols-2 gap-3">
                  <button
                    onClick={() => setIsLogoutOpen(false)}
                    className="rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm font-semibold text-zinc-700 hover:bg-zinc-50 dark:border-white/10 dark:bg-zinc-800 dark:text-zinc-300"
                  >
                    Cancel
                  </button>
                  <button
...
function HeaderIconButton({
  ...
  return (
    <button
      onClick={onClick}
      title={tooltip}
      className="group relative rounded-lg p-2.5 text-zinc-500 transition-colors hover:bg-zinc-100 dark:text-zinc-400 dark:hover:bg-white/10"
    >
      {children}
    </button>
  );
